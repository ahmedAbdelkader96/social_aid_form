...
export const DialCodeOptionList: FC<Props> = ({
  options,
  loading,
  noResultsText,
  ulClassName,
      ...
      {loading ? (
        ...
        options.map((entry) => (
          ...
          >
            ...
            <div>
              <span className={primaryClassName}>{entry.dialCode || getLocaleName(entry)}</span>
              {getLocaleName(entry) && (
                <span className={secondaryClassName} style={{ marginLeft: 8 }}>
                  {getLocaleName(entry)}
                </span>
              )}
            </div>
...
export default DialCodeOptionList
